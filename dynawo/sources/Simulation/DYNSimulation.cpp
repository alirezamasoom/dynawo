...
Simulation::compileModels() {
  ...
  if (hasEnvVar("DYNAWO_HEADER_FILES_FOR_PREASSEMBLED")) {
    string headerFileList = getEnvVar("DYNAWO_HEADER_FILES_FOR_PREASSEMBLED");
...
Simulation::simulate() {
  ...
  string tmpDir(getEnvVar("TMPDIR"));
  if (tmpDir == "")
    tmpDir = context_->getWorkingDirectory();

  stringstream fileName;
  fileName << tmpDir << "/.dynawoexec-" << pid_;
