...
static void generateModelFile(const string& modelName, const string& outputDir,
                              bool& withInitFile,
                              const string& additionalHeaderFiles);  ///< Rewrite parts of one whole Modelica model C/C++ code to fit Dynawo C/C++ requirements
...
int main(int argc, char ** argv) {
  ...
  desc.add_options()
          ...
          ("additionalHeaderFiles", po::value< string >(&additionalHeaderFiles),
  ...
  try {
    ...
    generateModelFile(modelName, outputDir1, withInitFile, additionalHeaderFiles);
...
void
generateModelFile(const string& modelName, const string& outputDir, bool& withInitFile, const string& additionalHeaderFiles) {
  string scriptsDir1 = getEnvVar("DYNAWO_SCRIPTS_DIR");
  string varExtCommand = "python " + scriptsDir1 + "/writeModel.py -m " + modelName + " -i " + outputDir + " -o " + outputDir;
  if (!additionalHeaderFiles.empty())
    varExtCommand += " -a " + additionalHeaderFiles;
  if (withInitFile)
    varExtCommand += " --init";
